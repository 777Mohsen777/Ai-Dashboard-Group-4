...
if uploaded_file:
    ...
    def plot_graph(ax, x, y, label, color, ylabel, title, failure_df, ylim):
    ...
    plot_graph(axs[0], 'Operating_Hours', 'Temperature', 'Engine Oil Temp', 'red', 'Temperature (°C)', 'Engine Temperature Performance', failure_temp, (0, 150))
    ...
    def create_gauge(title, value, min_val, max_val, ranges, colors):
    ...
    with col1:
        st.plotly_chart(create_gauge("Engine Temperature (°C)", temp_val, 0, 150,
                                     [(0, 70), (70, 80), (80, 100), (100, 120)], 
                                     ['lightgreen', 'yellow', 'orange', 'red']), use_container_width=True)
    with col2:
        st.plotly_chart(create_gauge("Chassis Vibration (g)", vib_val, 0, 2.5,
                                     [(0, 0.4), (0.4, 1), (1, 1.5), (1.5, 2)], 
                                     ['lightgreen', 'yellow', 'orange', 'red']), use_container_width=True)
    with col3:
        st.plotly_chart(create_gauge("Hydraulic Pressure (psi)", pres_val, 0, 400,
                                     [(0, 230), (230, 250), (250, 270), (270, 280), (280, 350), (350, 400)], 
                                     ['white', 'red', 'orange', 'yellow', 'lightgreen', 'black']), use_container_width=True)
