...
def component_zone(df):
    ...
    for comp in df['Component_Type'].unique():
        ...
        with g1:
            st.plotly_chart(gauge("Temp", comp_df['Temperature'], 0, 150, [((0, 70), "green"), ((70, 100), "yellow"), ((100, 120), "orange"), ((120, 150), "red")]), use_container_width=True, key=f"{comp}_gauge_temp"), comp_df['Temperature'], 0, 150,
                                  [((0, 70), "green"), ((70, 100), "yellow"), ((100, 120), "orange"), ((120, 150), "red")]), use_container_width=True)
        with g2:
            st.plotly_chart(gauge("Vib", comp_df['Vibration'], 0, 2.5, [((0, 0.4), "green"), ((0.4, 1), "yellow"), ((1, 1.5), "orange"), ((1.5, 2.5), "red")]), use_container_width=True, key=f"{comp}_gauge_vib"), comp_df['Vibration'], 0, 2.5,
                                  [((0, 0.4), "green"), ((0.4, 1), "yellow"), ((1, 1.5), "orange"), ((1.5, 2.5), "red")]), use_container_width=True)
        with g3:
            st.plotly_chart(gauge("Pres", comp_df['Pressure'], 0, 400, [((0, 230), "red"), ((230, 260), "orange"), ((260, 290), "yellow"), ((290, 320), "lightgreen"), ((320, 400), "black")]), use_container_width=True, key=f"{comp}_gauge_pres"), comp_df['Pressure'], 0, 400,
        ...
        c1, c2, c3 = st.columns(3)
        with c1: st.plotly_chart(dchart1, use_container_width=True, key=f"{comp}_donut_temp"), use_container_width=True)
        with c2: st.plotly_chart(dchart2, use_container_width=True, key=f"{comp}_donut_vib"), use_container_width=True)
        with c3: st.plotly_chart(dchart3, use_container_width=True, key=f"{comp}_donut_pres"), use_container_width=True)
