...
if uploaded_file:
    ...
    if st.button("Download Now"):
        ...
        with PdfPages(buffer) as pdf:
            ...
            # Add summary figure for selected hour gauges
            ...
            pdf.savefig(fig2)
        buffer.seek(0)
        st.download_button("📥 Save PDF", data=buffer, file_name="full_dashboard.pdf", mime="application/pdf")
...
st.markdown("#### 📸 Export Full Dashboard Screenshot")

async def capture_screenshot():
    browser = await launch(headless=True, args=['--no-sandbox'])
    page = await browser.newPage()

    # IMPORTANT: Replace this with your deployed app URL if not running locally
    url = "http://localhost:8501"

    await page.goto(url)
    await page.waitForSelector('body', timeout=10000)
    time.sleep(3)  # Ensure everything renders

    path = "full_dashboard.png"
    await page.screenshot({'path': path, 'fullPage': True})
    await browser.close()
    return path

if st.button("📥 Download Live Dashboard Image"):
    with st.spinner("Generating image from browser..."):
        try:
            screenshot_path = asyncio.run(capture_screenshot())
            with open(screenshot_path, "rb") as img:
                st.download_button("📸 Save PNG", data=img, file_name="dashboard.png", mime="image/png")
