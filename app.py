...
Please upload an Excel file (`.xlsx`) with **exactly these column headers**:
- `Operating Hour`
- `Temperature` (°C)
- `Vibration` (g)
- `Pressure` (psi)
...
⚠️ Make sure:
- ❌ The values in the cells **do not contain units** (e.g., `75`, not `75°C`).
- ✅ All units should match: °C for temperature, g for vibration, and psi for pressure.
- 📈 Data should be numeric only.
""")

template_path = "excel_template_correct_format.xlsx"
if os.path.exists(template_path):
    with open(template_path, "rb") as f:
        st.download_button("📥 Download Excel Template", f, file_name="excel_template_correct_format.xlsx")
else:
    st.warning("⚠️ Excel template file not found. Please contact the developer.")
...
def plot_sensor_data(df, failures):
    ...
    for i, (col, ylabel, color) in enumerate(metrics):
        ax = axs[i]
        ax.scatter(df["Operating Hour"], df[col], s=3, c=color, label=col)
        smooth = lowess(df[col], df["Operating Hour"], frac=0.02)
        ax.plot(smooth[:, 0], smooth[:, 1], color="black", lw=1.5, label="Performance")
        if not failures[i].empty:
            ax.scatter(failures[i]["Operating Hour"], failures[i][col],
...
required_columns = ["Operating Hour", "Temperature", "Vibration", "Pressure"]

if uploaded_file:
    df = pd.read_excel(uploaded_file).round(3)

    if not all(col in df.columns for col in required_columns):
        st.error(f"❌ Uploaded file is missing required columns. Found: {list(df.columns)}")
    elif not df[required_columns].applymap(lambda x: isinstance(x, (int, float, float))).all().all():
        st.error("❌ Some values contain non-numeric entries. Please remove any units from the cells.")
    else:
        ...
        df_clean = df.dropna()
        selected_hour = st.slider("Select Operating Hour", int(df_clean["Operating Hour"].min()),
                                  int(df_clean["Operating Hour"].max()))
        row = df_clean.iloc[(df_clean["Operating Hour"] - selected_hour).abs().argsort()[:1]]
