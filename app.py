...
if uploaded_file:
    ...
    if not expected.issubset(set(df.columns)):
        ...
        selected_hour = st.number_input("🔧 Select Specific Operating Hour", min_value=int(df["Operating Hour"].min()), max_value=int(df["Operating Hour"].max()), step=1)
        df_filtered = df[df["Operating Hour"] == selected_hour]
        if df_filtered.empty:
            st.warning(f"No data found exactly at hour {selected_hour}.")
        else:
            component_zone(df_filtered)
