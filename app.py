...
if uploaded_file:
    ...
    if not all(col in df.columns for col in required_columns):
    ...
    elif not df[required_columns].applymap(lambda x: isinstance(x, (int, float))).all().all():
        ...
        if any(pd.isnull([temp, vib, pres])):
            st.error("Invalid values detected at selected hour. Please try a different value.")
            st.stop()
