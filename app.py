...
def compute_rating(val, max_val):
    ratio = val / max_val
    if ratio >= 0.8:
        return ratio * 100, "green"
    elif ratio >= 0.6:
        return ratio * 100, "yellow"
    elif ratio >= 0.3:
        return ratio * 100, "orange"
    elif ratio > 0:
        return ratio * 100, "red"
    else:
        return 0, "black"

def ring_chart(title, percent, color):
    return go.Figure(go.Indicator(
        ...
        gauge = {
            ...
            "axis": {"range": [0, 100]},
            "bar": {"color": color},
            "threshold": {"line": {"color": color, "width": 2}, "value": percent}
        },
...
if uploaded_file:
    ...
    if not valid_df.empty:
        selected_hour = st.slider("Select Operating Hour", int(valid_df['Operating_Hours'].min()), int(valid_df['Operating_Hours'].max()))
        ...
        st.markdown("### 📊 Operating Hour Ratings")
        col4, col5, col6 = st.columns(3)
        temp_pct, temp_color = compute_rating(temp_val, 150)
        vib_pct, vib_color = compute_rating(vib_val, 2.5)
        pres_pct, pres_color = compute_rating(pres_val, 400)

        with col4:
            st.plotly_chart(ring_chart("Temperature Rating", temp_pct, temp_color), use_container_width=True)
        with col5:
            st.plotly_chart(ring_chart("Vibration Rating", vib_pct, vib_color), use_container_width=True)
        with col6:
            st.plotly_chart(ring_chart("Pressure Rating", pres_pct, pres_color), use_container_width=True)
