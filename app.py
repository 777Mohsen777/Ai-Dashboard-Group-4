...
st.title("📊 Equipment Sensor Dashboard (Compact)")
...
if uploaded_file:
    ...
    def plot_graph(ax, x, y, label, color, ylabel, title, failure_df):
        ax.scatter(df[x], df[y], color=color, s=4, label=label, alpha=0.6)
        smoothed = lowess(df[y], df[x], frac=0.02)
        ax.plot(smoothed[:, 0], smoothed[:, 1], color='black', linewidth=1.3, label='Performance')
    ...
    plot_graph(axs[0], 'Operating_Hours', 'Temperature', 'Engine Oil Temp', 'red', 'Temp (°C)', 'Engine Temp', failure_temp)
    plot_graph(axs[1], 'Operating_Hours', 'Vibration', 'Chassis Vibration', 'blue', 'Vibration (g)', 'Vibration', failure_vib)
    plot_graph(axs[2], 'Operating_Hours', 'Pressure', 'Hydraulic Oil Pressure', 'green', 'Pressure (psi)', 'Hydraulic Pressure', failure_pres)
    st.pyplot(fig)
    ...
    if not valid_df.empty:
        ...
        def create_gauge(title, value, min_val, max_val, ranges):
            ...
            fig.update_layout(margin=dict(t=5, b=5, l=5, r=5), height=200)
            return fig
