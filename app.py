...
async def capture_screenshot():
    ...
    # IMPORTANT: Replace with your deployed URL if hosted
    url = "http://localhost:8501"

    await page.goto(url)
    await page.waitForSelector('body', timeout=10000)
    await page.screenshot({'path': "full_dashboard.png", 'fullPage': True})
    await browser.close()
    return "full_dashboard.png"

def run_async_task():
    return asyncio.get_event_loop().run_until_complete(capture_screenshot())

if st.button("📥 Download Live Dashboard Image"):
    with st.spinner("Generating image from browser..."):
        try:
            screenshot_path = run_async_task()
