...
def component_zone(df):
    ...
    for comp in df['Component_Type'].unique():
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            st.plotly_chart(dchart1, use_container_width=True, key=f"{comp}_donut_temp")
        with c2:
            st.plotly_chart(dchart2, use_container_width=True, key=f"{comp}_donut_vib")
        with c3:
            st.plotly_chart(dchart3, use_container_width=True, key=f"{comp}_donut_pres")
