...
if uploaded_file:
    ...
    if not expected.issubset(set(df.columns)):
        ...
        selected_hour = st.number_input("🔧 Select Specific Operating Hour", 
                                        min_value=int(df["Operating Hour"].min()), 
                                        max_value=int(df["Operating Hour"].max()), 
                                        step=1)
        # For display only: find closest available hour
        ...
        df_selected = df[df["Operating Hour"] == closest_hour]
        component_zone(df)  # full data for graphs

        # Gauges and donuts only
        st.markdown("### 📍 Gauges and Health")
