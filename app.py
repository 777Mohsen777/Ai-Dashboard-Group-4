...
def line_chart(df, metric):
    ...
    df["Operating Hour"] = pd.to_datetime(df["Operating Hour"])
    for comp in df['Component_Type'].unique():
        ...
        ax.plot(comp_df["Operating Hour"], comp_df[f"{metric}_smooth"], label=comp, linewidth=1.8)
    ...
    for comp in df['Component_Type'].unique():
        ...
        ax.plot(comp_df['Operating Hour'], comp_df[metric], label=comp)
    ax.set_title(f"{metric} vs Operating Hour")
